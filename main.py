...
# f.write("\nlatest div = " + str(pfx(x, teta1)) + ' ' + str(pfx(x, teta0)) + " for x = " + str(x) + " for t0 = " + str(teta0) + " for t1 = " + str(teta1))
...
def drawplot2d_h1(mas, namex, namey, isWrongs):
    mas = np.array(mas)
    print(mas)
    # f.write(*mas)
    # f.write("\n")
    yval = np.linspace(0, 50, workNum)
    ...
    plt.xlabel(namex)
    plt.ylabel(namey)
    plt.savefig(namey + '.jpg')
    plt.show()


def drawplot2d_h2(mas, namex, namey):
    mas = np.array(mas)
    print(mas)
    yval = np.linspace(0, workNum, workNum)
...
def drawplot2d_h2s(mas0, mas1, xname, yname):
    mas0 = np.array(mas0)
    ...
    # print()
    plt.plot(yval, z0, label='H0='+str(teta0)+', H1='+str(teta1))
    plt.plot(yval, z1, label='H0='+str(teta0s)+', H1='+str(teta1s))
    plt.legend(loc='best')
    plt.xlabel(xname)
    plt.ylabel(yname)
    plt.savefig(yname + '.jpg')
    plt.show()


def drawplot2d_h2ss(mas0, mas1, mas2, xname, yname):
    mas0 = np.array(mas0)
    ...
    for tetaR in [teta0, teta1]:
        ...
        for eps in range(0, workNum):
            ...
            for i in range(0, n + 1):
                result = testNoisyBin(index, tetaR, float(eps / steps), k)
...
def test4(var, sub, k=10):
    ...
    for eps in range(0, workNum):
        ...
        obs = 0.
        for i in range(0, n + 1):
            result = testNoisyBin(var, tetaR, float(eps / steps), sub, k)
            # result = fun2(var, tetaR, float(eps / steps))
            # result = Test1(teta0, teta1)
            obs += result[1]
            if not result[0]:
...
def f4_1():
    print("\n****Верна первая гипотеза****\n")
    res = test4(0, 0)
    drawplot2d_h1(res[0], 'запуски теста 1', 'дополнительные наблюдения', False)
    drawplot2d_h1(res[1], 'запуски теста 1', 'ошибки 1го рода на 1000 итераций', True)
...
def f4_2():
    ...
    res1 = test4(1, 1)
    res2 = test4(1, 2)
    drawplot2d_h2(res[0], 'запуски теста 2', 'дополнительные наблюдения')
    drawplot2d_h2(res[1], 'запуски теста 2', 'ошибки 1го рода на 1000 итераций')
    drawplot2d_h2ss(res[0], res1[0], res2[0], 'запуски теста', 'дополнительные наблюдения')
    drawplot2d_h2ss(res[1], res1[1], res2[1], 'запуски теста', 'ошибки на 1000 итераций')
...
def testNoisyBin(scenario, tetaR, eps, sub, k=10):
    global A
    global B
    global logA
    global logB
